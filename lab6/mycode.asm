...
START:
    ...
    mov ah,9h
    int 21h
    call INPUT                                   ;вводим Enter time in protected mode
    mov ds:[TIME], al
    call FILL_CR_0_BUFFER_RM                 ;функция заполняет буфер BUFFER_CR_0_RM двоичным представлением значения, находящегося в регистре cr0
    lea dx, BUFFER_CR_0_RM                     ;Вывести содержимое рагистра CR0
    mov ah, 9h
    int 21h
    lea dx,MSG_HELLO
    mov ah,9h
...
PREPARE_RTC:                                    ;Подготовка часов RTC 
    mov al,0Bh ;0B регистр состояния
    out 70h,al                                  ;Выбрать регистр состояния 0Bh
    in  al,71h                                  
    or  al,00000100b                            ;Установить бит DM в 1 - формат представления время в двоичном виде
    out 71h,al                                  
ENABLE_A20:                                     ;Открыть линию A20(to get more address memory)Линия A20 - это линия адреса на шине данных, которая используется для доступа к памяти
                                                ;в защищенном режиме. При переходе на защищенный режим линия A20 должна быть открыта, чтобы система могла обращаться к памяти выше 1 МБ.
    in  al,92h                                                                              
    or  al,2                                    ;Установить бит 1 в 1                                                   
    out 92h,al      
    
SAVE_MASK:                                      ;Сохранить маски прерываний     ????????????????????????
    in      al,21h           ;читаем значение из 21h(это значение это маска ведущего )
    mov     INT_MASK_M,al                  
    in      al,0A1h                 ;читает значение маски ведомого контроллера
    mov     INT_MASK_S,al                 
DISABLE_INTERRUPTS:                             ;Запрет маскируемых и немаскируемых прерываний        
    cli                                         ;Запрет маскирумых прерываний
    in  al,70h	
	or	al,10000000b                            ;Установить 7 бит в 1 для запрета немаскируемых прерываний(немаскируемые это критичные прерывания)
	out	70h,al
	nop	                                        ;skip tackt
LOAD_GDT:                                       ;Заполнить глобальную таблицу дескрипторов            
    mov ax,DATA       ;Сегмент данных реального/защищенного режима
    mov dl,ah
    xor dh,dh
    shl ax,4;сдвиг влево
    shr dx,4
    mov si,ax
    mov di,dx
WRITE_GDT:                                      ;Заполнить дескриптор GDT. Регистр дескриптора глобальной таблицы дескрипторов (GDT) - это специальный регистр,
;используемый в процессорах семейства x86 для хранения таблицы дескрипторов. GDT содержит информацию о сегментах памяти, которые могут использоваться программой.
    lea bx,GDT_GDT
    mov ax,si
    mov dx,di
    add ax,offset GDT
    adc dx,0
    mov [bx][S_DESC.BASE_L],ax              ;младший
    mov [bx][S_DESC.BASE_M],dl          ;средний
    mov [bx][S_DESC.BASE_H],dh        ;старший
...
WRITE_CODE_PM:                                  ;Записать дескриптор кода защищенного режима
    lea bx,GDT_CODE_PM
    mov ax,CODE_PM          ;сюда заносится сегментный селектор
    xor dh,dh
    mov dl,ah
    shl ax,4
    shr dx,4
...
FILL_IDT:                                       ;Заполнить таблицу дескрипторов векторов прерываний
    ...
    ;Директива irpc — это макрос, который выполняет итерацию по заданному списку символов и заменяет параметр N каждым символом по очереди.
        lea eax, EXC_0&N
    ...
    ;за обработку IRG0(таймер) отвечает 20 шлюз
    ...
    ;GDTR и IDTR - это регистры, которые содержат адреса таблиц глобальных дескрипторов (GDT) и таблиц дескрипторов прерываний (IDT) соответственно.
    mov eax,cr0                                 ;Получить управляющий регистр cr0

;Регистр CR0 (Control Register 0) является одним из регистров контроля процессора в архитектуре x86. Он управляет различными аспектами работы процессора,
;такими как защита памяти, включение/выключение кэша и использование защищенного режима процессора.а нулевой бит РЕ управляет on/off защищенного режима
...
PREPARE_TO_RETURN:                              ;Подготовится к возврату в реальный режим
    ...
    push edi                                    
    ;Но в защищённом режиме номера от 08h до 0Fh зарезервированы для oбработки исключений!поэтому
...
DATA_BEGIN      = $
    ...
    R_IDTR  ends
    ;______________________________________________________________________________|
    ;Использование селекторов сегментов вместе с глобальной таблицей дескрипторов позволяет операционной системе эффективно управлять доступом к памяти и защищать систему от несанкционированного доступа к памяти
    ;GDT - глобальная таблица дескрипторов
    ;Глобальная таблица дескрипторов (GDT) - это таблица, которая хранит дескрипторы сегментов памяти в x86 архитектуре. Она используется для управления доступом к памяти, задания прав доступа к сегментам памяти,
    ;а также для управления прерываниями и системными вызовами.
    ;в дескриптор(8байт)входят адрес размер и др
    ; GDT содержит дескрипторы сегментов, которые определяют базовый адрес и размер сегмента, а также другие атрибуты, такие как уровень привилегий и тип сегмента
;GDT необходима для того, чтобы операционная система могла эффективно управлять памятью и процессами, а также для обеспечения безопасности системы. Каждый дескриптор содержит информацию о размере сегмента, его базовом адресе и правах доступа к этому сегменту.
    ;__Каждый дескриптор содержит информацию о базовом адресе сегмента, его размере, уровне привилегий и других параметрах. 
    ;Эти дескрипторы используются для определения доступных сегментов памяти в защищенном режиме процессора x86.____________________
    GDT_BEGIN   = $;метку начала таблицы
    GDT label   word                            ;Метка начала GDT
    GDT_0       S_DESC <0,0,0,0,0,0>             ;определяет нулевой дескриптор сегмента.                     
    GDT_GDT     S_DESC <GDT_SIZE-1,,,10010010b,0,>     ;определяет дескриптор таблицы дескрипторов Флаги сегмента установлены таким образом, что сегмент является доступным для чтения и записи и имеет уровень привилегий 0.            
    GDT_CODE_RM S_DESC <SIZE_CODE_RM-1,,,10011010b,0,>  ;определяет дескриптор сегмента кода в режиме реального режима   (ч)        
    GDT_DATA    S_DESC <SIZE_DATA-1,,,11110010b,0,>      ;определяет дескриптор сегмента данных. (ч з)
    GDT_STACK   S_DESC <1000h-1,,,10010010b,0,>   ;определяет дескриптор сегмента данных. (ч)                 
    GDT_TEXT    S_DESC <2000h-1,8000h,0Bh,11110010b,0,0> ;определяет дескриптор сегмента текста. (ч з)
    GDT_CODE_PM S_DESC <SIZE_CODE_PM-1,,,10011010b,01000000b,>    ;определяет дескриптор сегмента кода в защищенном режиме(ч)
    GDT_IDT     S_DESC <SIZE_IDT-1,,,10010010b,0,>   ;определяет дескриптор таблицы векторов прерываний (ч)               
    GDT_SIZE    = ($ - GDT_BEGIN)               ;Размер GDT
    ;______________________________________________________________________________|

    ;Селлекторы сегментов  это 16-битные значения, которые используются в x86-совместимых процессорах для доступа к сегментам памяти, которые определены в таблице дескрипторов. Каждый селектор сегмента указывает на дескриптор сегмента в таблице дескрипторов.
    ...
    ;IDT - таблица дескрипторов прерываний это таблица, используемая в x86-совместимых процессорах для обработки прерываний и исключений. Каждый дескриптор в таблице содержит информацию о прерывании, включая адрес обработчика прерывания и уровень привилегий, необходимый для его выполнения.
    ;______________________________________________________________________________
    ...
    ENDM
    IDT_TIMER    I_DESC <0,CODE_PM_DESC,0,10001110b,0>             ;IRQ 0 - прерывание системного таймера IDT_TIMER - это метка (label) для описателя прерывания, который используется для обработки прерывания от таймера системы.

;I_DESC - это директива ассемблера, которая создает описатель прерывания в таблице описателей прерываний (IDT).

;<0,CODE_PM_DESC,0,10001110b,0> - это список параметров для описателя прерывания. Он включает в себя следующие поля:

;- Поле сегмента нулевое, что означает, что описатель использует текущий сегмент кода.
;- CODE_PM_DESC - это селектор сегмента кода, который указывает на начало обработчика прерывания в памяти.
;- Поле смещения равно 0, так как обработчик прерывания начинается с начала сегмента кода.
;- Флаги установлены в 10001110b, что означает следующее:
 ; - бит 0 - это флаг присутствия (present), который указывает, что описатель прерывания действителен.
  ;- биты 1-3 - зарезервированные биты, которые должны быть установлены в 0.
  ;- бит 4 - это флаг размера (size), который указывает на размер обработчика прерывания (в данном случае 32-битный).
  ;- бит 5 - это флаг типа (type), который указывает на тип прерывания (в данном случае это прерывание процессорного времени).
  ;- бит 6 - это флаг уровня привилегий (DPL), который указывает на уровень привилегий обработчика прерывания (в данном случае это уровень 0).
  ;- бит 7 - это флаг присутствия (present), который повторяет бит 0.
