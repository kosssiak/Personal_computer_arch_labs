...
void set_delay(unsigned int delay)
{
	...
	do
	{
		outp(0x70, 0xA); 						// выбор регистра A (проверка на занятость часов (7 бит))
        	check = inp(0x71) & 0x80;					// 		1		000 			   0000
        									// 	   часы заняты	  делитель фазы       выбор частоты период. прер.
	}while(check);							
	outp(0x70, 0xA);
    ...
    outp(0x70, 0xB); 								// выбор регистра В
    outp(0x71, inp(0x71) | 0x40); 						// 0x40 = 	0		1		 0		     0 		     0		     0		  0
    										//        запрет обновл.   вызов период.    вызов прервыв.      вызов прер.      включена генер.   формат даты  переход
										//            часов	   прерыв.(IRQ8)  при сраб. будильн. по оконч. обновл. прямоугольн. имп.  и времени    на летн.
									 	//                         	                                  времени                                       время
    delay_count = 0;
...
void set_alarm()
{
    ...
    old_alarm_interrupt = getvect(0x70); 					// 0x70 - обновление времени
...
void reset_alarm()
{
    ...
    setvect(0x70, old_alarm_interrupt); 
...
{
    outp(0x70, 0x0C);
	if (inp(0x71) & 0x40) 							// 			0 			1		           0		     0 		        0000
	{									// 		произошло прер.       разрешено период. прер.        разрешено прер.  разрешено прер. по оконч. 
										//								     от будильника     обновления часов
		delay_count++;
