...
void interrupt  newint08(...) { changeColor(); print(); oldint8(); }                     
...
void initialize()
{
	...
	//IRQ 0-7
	setvect(0xB8, newint08);  
	setvect(0xB9, newint09);  
	setvect(0xBA, newint0A);  
	setvect(0xBB, newint0B);  
	setvect(0xBC, newint0C);  
	setvect(0xBD, newint0D);  
	setvect(0xBE, newint0E);  
	setvect(0xBF, newint0F);  
	...
	outp(0x20, 0x11);	//ICW1 - инициализация ведущего контроллера (00 	1			0			0			0		          1)	
	...
	outp(0x21, 0xB8);   	//ICW2 - инициализация ведущего контроллера 									(01110			000)	
				//	установка адреса вектора прерывания	для первого контроллера нужно значение B8h    		     используются	  не используются
				//	  	для IRQ0 или IRQ8		для второго - 08h		
